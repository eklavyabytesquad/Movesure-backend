...
bilty for that transport across two sources:
  1. bilty table              (uses bilty_date)   → type = "regular"
  2. station_bilty_summary    (uses created_at)   → type = "manual"

Response structure:
{
  transport_name, transport_gstin, from_date, to_date, sources, summary,
  with_pohonch: {
    "<pohonch_number>": {
  ...
  no_pohonch: {
    "<challan_no>": [ <bilty>, ... ],
    "UNKNOWN": [ <bilty with no challan> ]
  }
}

Each bilty includes challan_no + challan_dispatch_date.
"""
...
def _natural_key(s: str):
    """Natural sort key so HC0002 < HC0010 etc."""
...
def get_transport_bilty_report(
    ...
    # ── 1. Fetch bilties from both tables ─────────────────────────────────────
    ...
    # Merge — bilty table wins on duplicate gr_no
    seen = set()
    unified = []
    ...
    # ── 2. bilty_wise_kaat — includes challan_no ──────────────────────────────
    kaat_map = _fetch_kaat(sb, gr_nos)

    # ── 3. Pohonch → gr_no maps ───────────────────────────────────────────────
    ...
    # ── 4. Challan dispatch dates ─────────────────────────────────────────────
    all_challan_nos = list({
        row["challan_no"] for row in kaat_map.values()
    ...
    # ── 6. Build enriched rows & group ────────────────────────────────────────
    # with_pohonch:  { pohonch_number -> { "regular": [...], "manual": [...] } }
    # no_pohonch:    { challan_no     -> [ ... ] }
    with_pohonch_groups: dict[str, dict[str, list]] = defaultdict(lambda: {"regular": [], "manual": []})
    no_pohonch_groups:   dict[str, list]             = defaultdict(list)
    ...
    for b in unified:
        ...
        row = {
            "source":               b["source"],         # "regular" | "manual"
            "gr_no":                gr,
        ...
        if pohonch_number:
            ...
            # priority: challan_no → bilty_number (transit bilty) → UNKNOWN
            bilty_number = _safe(kaat.get("bilty_number"))
            key = challan_no if challan_no else (bilty_number if bilty_number else "UNKNOWN")
            no_pohonch_groups[key].append(row)

    # Sort within each group by gr_no ascending
    for pno, grp in with_pohonch_groups.items():
        grp["regular"].sort(key=lambda x: x["gr_no"])
        grp["manual"].sort(key=lambda x: x["gr_no"])

    for cno, lst in no_pohonch_groups.items():
        lst.sort(key=lambda x: x["gr_no"])

    # Natural sort: HC0001, HC0002, …, HC0010, HC0011
    with_pohonch_sorted = dict(sorted(with_pohonch_groups.items(), key=lambda kv: _natural_key(kv[0])))
    # Challan groups: numeric natural sort, UNKNOWN always last
    def _challan_sort_key(kv):
        k = kv[0]
        if k == "UNKNOWN":
            return (1, [])
        return (0, _natural_key(k))
    no_pohonch_sorted = dict(sorted(no_pohonch_groups.items(), key=_challan_sort_key))

    with_pohonch_count = sum(
        len(g["regular"]) + len(g["manual"]) for g in with_pohonch_sorted.values()
    )
    without_pohonch_count = sum(len(v) for v in no_pohonch_sorted.values())

    transport_name_resolved = (unified[0]["transport_name"] if unified else transport_name or "")
    ...
    return {
        ...
        "to_date": to_date,
        "transport_gstin": transport_gstin or "",
        "transport_name": transport_name_resolved,
        "sources": {
...
def _fetch_challan_dispatch(sb, challan_nos: list) -> dict:
    """Returns { challan_no -> challan detail dict }"""
    dispatch_map = {}
    if not challan_nos:
        return dispatch_map
    for chunk in _chunks(challan_nos, 100):
        res = (
            ...
            .select(
                ...
                "is_received_at_hub, received_at_hub_timing, remarks, "
                "total_bilty_count"
            )
...
def _fetch_pohonch_maps(sb, transport_filter, is_gstin):
    ...
    while True:
        ...
        q = q.eq("transport_gstin", transport_filter.strip().upper()) if is_gstin \
            else q.ilike("transport_name", f"%{transport_filter.strip()}%")
        batch = q.execute().data or []
        for p in batch:
