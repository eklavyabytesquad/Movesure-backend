"""
GSTIN Details Service
Fetches GSTIN details (trade name, legal name, address, status, etc.)
...
def get_gstin_details(user_gstin, gstin):
    """
    Get GSTIN Details for a given GSTIN number.

    Args:
        user_gstin: The logged-in user's GSTIN
        gstin:      The GSTIN to look up

    Returns:
        dict: Structured response with GSTIN details or error info
    """
    # Get auth headers
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            if api_status == "No Content" or api_code == 204:
                ...
                return {
                    "status": "error",
                    "message": api_message if isinstance(api_message, str) else "Could not retrieve GSTIN details",
                    "nic_code": nic_code,
                    "data": result,
                }
            ...
            print(f"✅ GSTIN details retrieved for {details.get('gstin_of_taxpayer', gstin)}")
            ...
            return {
                "status": "error",
                "message": "Failed to retrieve GSTIN details",
                "error": error_data,
