...
REQUIRED_FIELDS = [
    ...
    'total_invoice_value', 'itemList'
    # transportation_mode and transportation_distance are optional for Part A only generation
]
...
def validate_payload(data):
    ...
    # When document_type is 'Others', sub_supply_description is required by NIC
    if doc_type == 'Others':
        desc = str(data.get('sub_supply_description', '')).strip()
        if not desc:
            errors.append("sub_supply_description: Required when document_type is 'Others' (describe the nature of the document)")
    ...
    # transportation_mode — empty string means Part A only (vehicle updated later via Part B)
    transport_mode = str(data.get('transportation_mode', '')).strip()
    if transport_mode and transport_mode not in VALID_TRANSPORT_MODES:
        errors.append(f"transportation_mode: Must be one of {VALID_TRANSPORT_MODES} or empty for Part-A only, got '{transport_mode}'")

    # vehicle_number: required for Road mode unless transporter_id is provided (transporter updates vehicle later)
    if transport_mode == 'Road':
        vehicle = str(data.get('vehicle_number', '')).strip()
        has_transporter = bool(str(data.get('transporter_id', '')).strip())
        if not vehicle and not has_transporter:
            errors.append("vehicle_number: Required when transportation_mode is 'Road' (or provide transporter_id to update vehicle later)")
        elif vehicle and not VEHICLE_REGEX.match(vehicle):
    ...
    if not isinstance(item_list, list) or len(item_list) == 0:
        ...
        for idx, item in enumerate(item_list):
            ...
            # Auto-fix negative tax rates (-1 means "not applicable" in govt data)
...
def normalize_payload(data):
    """
    Normalize field names and auto-fill fields that NIC requires but callers often omit.

    Key NIC requirements learned from live testing:
    - actual_from_state_name and actual_to_state_name are required by NIC even when
      they match state_of_consignor / state_of_supply. Auto-fill from those if absent.
    - transaction_type defaults to 1 (Regular) if not supplied.
    - generate_status=1 and data_source='erp' must be present.
    - sub_supply_description must be present (empty string is fine) for 'Others' doc type.
    """
    # snake_case → camelCase/mixedCase aliases
    field_map = {
    ...
    # Auto-fill actual dispatch / destination state from billing state if not provided.
    # NIC validates pincode against actual_from/to_state_name — omitting them causes
    # "Enter the correct actual state of consignor" errors even when other state fields
    # are correct.
    if not data.get('actual_from_state_name') and data.get('state_of_consignor'):
        data['actual_from_state_name'] = data['state_of_consignor']
    if not data.get('actual_to_state_name') and data.get('state_of_supply'):
        data['actual_to_state_name'] = data['state_of_supply']

    # Default transaction_type to 1 (Regular) — Bill To and Ship To are the same party.
    data.setdefault('transaction_type', 1)

    # NIC requires these housekeeping fields; defaults are safe for all ERP-generated EWBs.
    data.setdefault('generate_status', 1)
    data.setdefault('data_source', 'erp')

    # sub_supply_description must be present for 'Others' document type (NIC rule).
    # Default to empty string so callers don't need to send it explicitly.
...
def generate_delivery_challan_ewaybill(data):
    """
    Convenience wrapper for generating a Delivery Challan EWB.

    Pre-sets these defaults so the caller only needs to supply the
    shipment-specific fields (parties, amounts, items, transport):
        document_type        = "Delivery Challan"
        sub_supply_type      = "Others"
        sub_supply_description = "Delivery against invoice <document_number>"

    All other fields (actual_from_state_name, transaction_type, etc.)
    are still auto-filled by normalize_payload inside generate_ewaybill.
    """
    data.setdefault('document_type', 'Delivery Challan')
    data.setdefault('supply_type', 'outward')
    data.setdefault('sub_supply_type', 'Others')
    if not data.get('sub_supply_description'):
        doc_num = data.get('document_number', '')
        data['sub_supply_description'] = f"Delivery against invoice {doc_num}".strip()
    return generate_ewaybill(data)
...
def generate_ewaybill(data):
    ...
    try:
        ...
        print(f"Total Value:     {data.get('total_invoice_value')}")
        ...
        # MastersIndia wraps NIC errors inside HTTP 200; also handle 206 the same way
        if response.status_code in (200, 201, 206):
            ...
            # NIC error comes as a 200/206 with status="No Content" or code=204
            if api_status == "No Content" or str(api_code) == "204" or api_code == 204:
                nic_code = api_results.get("nic_code", "")
                error_text = api_message if isinstance(api_message, str) else "API returned an error"
                print(f"NIC error: {error_text}")
                return {
                    "status": "error",
                    "message": error_text,
                    "nic_code": nic_code,
            ...
            # Success — extract key fields from the nested message dict
            ...
            print(f"E-Way Bill generated: {ewb_no}")
            ...
            print(f"Failed — HTTP {response.status_code}: {json.dumps(error_data, indent=2)}")
            return {
                "status": "error",
